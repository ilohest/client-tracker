...
const draggedSubItem = ref<{ addonId: string; itemId: string; subItemId: string } | null>(null);
...
const startSubItemDrag = (addonId: string, itemId: string, subItemId: string) => {
  draggedSubItem.value = { addonId, itemId, subItemId };
};

const handleItemDragOver = (addonId: string, targetId: string, event: DragEvent) => {
...
const handleSubItemDragOver = (addonId: string, itemId: string, targetId: string, event: DragEvent) => {
  event.preventDefault();
  dropState.value = { addonId, targetType: 'subitem', parentItemId: itemId, targetId, mode: 'before' };
};

const dropItem = (addonId: string, targetId: string) => {
  if (draggedItem.value && draggedItem.value.addonId === addonId && draggedItem.value.itemId !== targetId) {
    if (dropState.value?.targetType === 'item' && dropState.value.mode === 'nested') {
      emit('nestAddonItemUnderItem', { addonId, draggedId: draggedItem.value.itemId, targetId });
    } else {
      emit('moveAddonItem', { addonId, draggedId: draggedItem.value.itemId, targetId });
    }
...
const dropSubItem = (addonId: string, itemId: string, targetId: string) => {
  ...
  if (draggedSubItem.value.addonId !== addonId || draggedSubItem.value.itemId !== itemId || draggedSubItem.value.subItemId === targetId) return;
  emit('moveAddonSubItem', { addonId, itemId, draggedId: draggedSubItem.value.subItemId, targetId });
  clearDragState();
};
...
const isSubItemDropBefore = (addonId: string, itemId: string, subItemId: string) =>
  dropState.value?.addonId === addonId &&
...
const syncExpandedAddons = (addons: QuoteAddon[]) => {
  ...
  expandedAddonIds.value = addons.filter((addon) => existing.has(addon.id)).map((addon) => addon.id);

  addons.forEach((addon) => {
    if (!existing.has(addon.id)) {
      expandedAddonIds.value.push(addon.id);
    }
  });
};
...
const isAddonExpanded = (addonId: string) => expandedAddonIds.value.includes(addonId);

const toggleAddon = (addonId: string) => {
  if (isAddonExpanded(addonId)) {
    expandedAddonIds.value = expandedAddonIds.value.filter((id) => id !== addonId);
    return;
  }
  ...
  <div class="mt-6 rounded-3xl border border-surface-dark/5 bg-white p-5">
    <div class="mb-4 flex items-center justify-between gap-3">
      ...
      <Button severity="secondary" @click="emit('addAddon')">
        <template #icon><span class="material-symbols-outlined text-lg">playlist_add</span></template>
        Ajouter un add-on
      </Button>
    </div>

    <div v-if="addons.length === 0" class="rounded-2xl border border-dashed border-surface-dark/10 p-5 text-sm text-surface-dark/55">
    ...
    <div v-else class="flex flex-col gap-4">
      <div
        ...
        :class="draggedAddonId === addon.id ? 'shadow-lg ring-2 ring-primary/20' : ''"
        @dragover.prevent
        ...
        <div class="flex items-center justify-between gap-3">
          <div class="flex min-w-0 items-center gap-3">
            ...
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0">
                <p class="truncate text-sm font-semibold text-surface-dark">{{ addon.title || `Add-on ${index + 1}` }}</p>
                <p v-if="!isAddonExpanded(addon.id)" class="mt-0.5 text-xs text-surface-dark/55">
          ...
          <div class="flex items-center gap-1">
            <Button text severity="secondary" @click="toggleAddon(addon.id)">
              <template #icon>
            ...
            <Button text severity="danger" @click="emit('removeAddon', addon.id)">
              <template #icon><span class="material-symbols-outlined text-lg">delete</span></template>
            </Button>
          </div>
        </div>

        <div v-if="isAddonExpanded(addon.id)" class="mt-4 mb-3 grid grid-cols-1 items-start gap-3 lg:grid-cols-[minmax(0,1fr)_180px]">
          <div class="min-w-0">
            <InputText class="w-full" :model-value="addon.title" @update:model-value="emit('updateAddon', { id: addon.id, field: 'title', value: $event || '' })" />
          </div>
          <div class="min-w-0">
            <InputNumber
              ...
              @update:model-value="emit('updateAddon', { id: addon.id, field: 'price', value: Number($event || 0) })"
            />
          </div>
        </div>

        <div v-if="isAddonExpanded(addon.id)" class="rounded-3xl border border-surface-dark/8 bg-white p-4" @dragleave="handleContainerDragLeave">
          <div v-if="addon.items?.length" class="space-y-3">
            <div
              ...
              :class="[
                ...
                isItemDropNested(addon.id, item.id) ? 'bg-primary/5 ring-2 ring-primary/15' : '',
              ]"
            ...
            >
              <div v-if="isItemDropBefore(addon.id, item.id)" class="mb-3 h-0.5 rounded-full bg-primary"></div>
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-start gap-3">
                    <Textarea
                      ...
                      rows="2"
                      auto-resize
                      @update:model-value="emit('updateAddonItem', { addonId: addon.id, itemId: item.id, value: $event || '' })"
                    />
                    <div class="flex shrink-0 flex-col items-center gap-1 pt-1">
                      <Button text severity="danger" class="!h-10 !w-10 !rounded-xl" @click="emit('removeAddonItem', { addonId: addon.id, itemId: item.id })">
                        <template #icon><span class="material-symbols-outlined text-lg">delete</span></template>
                      </Button>
                    </div>
                  </div>
                  ...
                  <div v-if="item.subItems.length" class="mt-4 ml-5 pl-8">
                    <div class="space-y-3">
                      <div
                        ...
                        @dragstart="startSubItemDrag(addon.id, item.id, subItem.id)"
                        @dragend="handleDragEnd"
                        @dragover="handleSubItemDragOver(addon.id, item.id, subItem.id, $event)"
                        @drop="dropSubItem(addon.id, item.id, subItem.id)"
                      >
                        <div v-if="isSubItemDropBefore(addon.id, item.id, subItem.id)" class="mb-3 h-0.5 rounded-full bg-primary"></div>
                        <div class="flex items-start gap-3">
                          ...
                          <Textarea
                            ...
                            rows="2"
                            auto-resize
                            @update:model-value="emit('updateAddonSubItem', { addonId: addon.id, itemId: item.id, subItemId: subItem.id, value: $event || '' })"
                          />
                          <div class="flex shrink-0 flex-col items-center gap-1 pt-1">
                            <Button text severity="danger" class="!h-10 !w-10 !rounded-xl" @click="emit('removeAddonSubItem', { addonId: addon.id, itemId: item.id, subItemId: subItem.id })">
                              <template #icon><span class="material-symbols-outlined text-lg">delete</span></template>
                            </Button>
                          </div>
                        </div>
                    ...
                    <Button text severity="secondary" class="mt-3 w-full justify-start rounded-2xl border border-surface-dark/8 bg-white px-4 py-3" @click="emit('addAddonSubItem', { addonId: addon.id, itemId: item.id })">
                  ...
                  <Button
                    ...
                    @click="emit('addAddonSubItem', { addonId: addon.id, itemId: item.id })"
                  >
                    <template #icon><span class="material-symbols-outlined text-lg">add_circle</span></template>
                    Ajouter un sous-point
                  </Button>
          ...
          <p v-else class="text-sm text-surface-dark/55">Aucun point pour cet add-on.</p>

          <Button text severity="secondary" class="mt-4 w-full justify-start rounded-2xl border border-surface-dark/8 bg-surface-light px-4 py-3" @click="emit('addAddonItem', addon.id)">
            <template #icon><span class="material-symbols-outlined text-lg">add_circle</span></template>
            Ajouter un point
          </Button>
        </div>
